...
{
    public class TaskInProgressUseCaseTests
    {
        [Fact]
        public async Task ShouldInProgressTask()
        {
            ...
            var useCase = new TaskInProgressUseCase(mockDatabase);

            var task = await mockDatabase.Tasks.FirstOrDefaultAsync();

            await useCase.Execute(task.Id);

            var taskInProgress = await mockDatabase.Tasks.FirstOrDefaultAsync();

            Assert.Equal(Status.InProgress, taskInProgress.Status);
